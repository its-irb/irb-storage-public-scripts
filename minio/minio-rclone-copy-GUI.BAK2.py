...
def abrir_interfaz_copia(root, perfil_rclone):
    ...
    # -------- METADATA FIELDS --------
    frame_metadata = ttk.LabelFrame(ventana, text="Metadata (for audit/log purposes)")
    ...
    for idx, (label_text, var_name) in enumerate(labels):
        ...
        metadata_vars[var_name] = entry  # Guardamos los widgets para leerlos después
...
def seleccionar_servidor_minio():
    ...
    # Usamos Toplevel para evitar conflicto si ya hay un root
    ventana = tk.Toplevel()
    ventana.title("Seleccionar servidor MinIO")
    ventana.geometry("400x300")  # Un poco más de alto para botón extra
    ventana.update_idletasks()
    ...
    ventana.grab_set()  # Bloquea otras ventanas hasta cerrar esta
    ventana.wait_window()  # Espera hasta que se cierre
    return resultado
...
def prompt_credenciales_renovar(tiempo_restante: str):
    ...
    label = tk.Label(ventana, text="Remaining lifespan for current credentials:", font=("Arial", 12))
...
def pedir_credenciales_irb(root) -> dict:
    ...
    Tk_Width = 350
    Tk_Height = 150
    x_Left = int(ventana.winfo_screenwidth()/2 - Tk_Width/2)
    y_Top = int(ventana.winfo_screenheight()/2 - Tk_Height/2)
    ...
    # Acción al pulsar botón o Enter
    def submit():
