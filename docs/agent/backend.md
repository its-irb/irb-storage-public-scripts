...
| **Mount/unmount** | `obtener_letra_unidad_disponible()` (Windows), `generar_punto_montaje()`, `montar_share_rclone()` (CIFS, transfer web), `desmontar_todos_los_shares(usuario)`, `desmontar_punto_montaje(mount_point, log_fn)`, `mount_rclone_S3_prefix_to_folder()` (S3, mount), `desmontar_todos_los_mounts_s3()`, `resolver_mount_point_destino()`. **Invariante: todos los montajes son read-only.** `mount_rclone_S3_prefix_to_folder` lanza `rclone mount ... --read-only --links` (`backend.py:737`); `montar_share_rclone` añade `--vfs-cache-mode off --read-only` (`backend.py:1209-1210`). No habilitar escritura en montajes S3 ni CIFS: Bifrost Mount es solo lectura por diseño (para subir datos se usa Bifrost Transfer), y los mounts CIFS del cluster sirven únicamente como origen de copia. |
