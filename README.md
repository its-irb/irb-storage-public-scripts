...
bifrost-mount/          # S3 bucket mounting app
  src/
    ...
    config.py           # APP_INFO for this app
    version.py
...
`flet build` uses the parameters defined in each app's `pyproject.toml`. Each app references the shared backend as `bifrost-shared @ file:///__BUILDPATH__/shared`; the `__BUILDPATH__` placeholder is replaced with the real path (or the generated wheel path) by the build scripts before building, and reverted afterwards.

**Local builds:**

- **macOS** — run `bifrost-mount/build-macos.sh` or `bifrost-transfer/build-macos.sh` from the app folder.
- **Windows** — run `.\build-local.ps1 -app bifrost-mount` (or `bifrost-transfer`) from the repo root. This generates the `bifrost-shared` wheel, substitutes it in `pyproject.toml`, runs `flet build windows`, and reverts the change.

**CI** (`.github/workflows/main.yml`) builds three platforms in parallel for both apps:

| Platform | Tool | Artifact |
|---|---|---|
| macOS | `flet build macos` + create-dmg | `.dmg` |
| Windows | `flet build windows` + Inno Setup | `.exe` |
| Rocky Linux 9 | PyInstaller onefile | `-linux` binary |

Rocky Linux uses PyInstaller onefile because `flet build linux` only supports Debian/Ubuntu. The Windows installer uses Inno Setup (config in `installer.iss`), which packages the entire `flet build` output into a single `.exe`. The version is injected as `1.0.<run_number>` in `src/version.py` and `pyproject.toml`.

If Python dependencies are updated, regenerate and import them:
```bash
